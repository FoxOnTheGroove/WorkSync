...
class EbsSimulateService:
    ...
    def collide(cls):
        ...
        _meetings         쌍((EBS 메시, 장비 메시))마다 한 번만 검사한다.
                          ...
                          닿은 자리도 조각별로 같이 준다 -- 빨간 상자가 설
                          자리다. 두 삼각형의 상자가 겹치는 데만 쓴다:
                          삼각형 하나도 클 수 있어서(긴 기둥 옆면 한 장이
                          기둥 전체를 덮는다) 만난 삼각형 통째로 쓰면 길게
                          뻗는다. 한 자리를 찾아도 그 쌍을 안 끊는다 --
                          떨어진 기둥 둘이 한 메시면 한쪽만 표시된다.
                          대신 자리 수를 CLASH_SPOTS 로 막는다
        _widen            겹치는 자리끼리 묶는다. 자라면서 닿게 된 옛 자리도
                          같이 삼킨다 -- 한 번만 보고 넘기면 같은 자리에
                          상자가 여러 개 겹쳐 선다
        _clash_boxes      걸린 조각을 빨간 반투명 상자로 감싼다. 모서리
                          선은 없다 -- 조각이 많이 겹치면 선이 더 뭉갬
                          상자는 조각 전체가 아니라 _meetings 가 준 '닿은
                          자리'다. 조각 전체를 쓰면 멀리 떨어진 부분 둘이 한
                          메시일 때 그 사이 빈 곳까지 덮는다. 삼각형이 없는
                          프리미티브만 제 상자로 떨어진다
                          CLASH_OPACITY / COLOR_CLASH / CLASH_MARKS / CLASH_MIN
