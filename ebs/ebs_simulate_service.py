...
class EbsSimulateService:
    ...
    def collide(cls):
        ...
        _flat_gap         메시 면이 삼각형 여러 개(같은 높이=한 면)여도
                          ...
                          '같은 평면'인지는 _flat_slack 만큼 봐준다 (FLAT_TOL,
                          그 조각 폭 대비). 딱 떨어지는 값만 한 면으로 치면
                          조금 기운 면은 모서리로 쏠린다
        _joined           높이가 같아도 붙어 있는 것만 한 면이다. 가장 가까운
                          삼각형에서 꼭짓점을 타고 이어지는 것만 모은다 --
                          떨어진 두 면이 우연히 같은 높이일 때 묶으면 그 사이
                          허공에 선이 간다
