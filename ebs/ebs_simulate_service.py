...
class EbsSimulateService:
    ...
    @classmethod
    def attach(cls, simulate) -> None:
        """서비스가 들 EbsSimulate 하나를 건다. 익스텐션이 시작할 때 부른다"""
    ...
    async def simulate(cls, equipment=""):
        ...
        return await cls._simulate.run_simulate(equipment)

    @classmethod
    async def clear(cls):
        """그린 것, 레이저, 카메라, EBS, 머티리얼을 전부 놓는다"""
        return await cls._simulate.run_clear()
    ...
    def busy(cls):
        ...
        return cls._simulate.busy() if cls._simulate else ""
